...
describe('PollServiceService', () => {
  ...
  it('should poll a service 3 times and raise an error', () => {
    scheduler.run(helpers => {
      const source$ = of(1);
      const result$ = source$.pipe(service.poll(
        () => true, 3, undefined, 2, false));
      const expected = '1-1-1-#';

      helpers.expectObservable(result$).toBe(expected,
        { 1: 1 }, Error('Failed to fetch data'));
    });
  });
  ...
  it('should poll a service 3 times and raise a custom error', () => {
    scheduler.run(helpers => {
      const source$ = of(1);
      const result$ = source$.pipe(service.poll(
        () => true, 3, 'Test error message', 2, false));
      const expected = '1-1-1-#';

      helpers.expectObservable(result$).toBe(expected,
        { 1: 1 }, Error('Test error message'));
    });
  });
